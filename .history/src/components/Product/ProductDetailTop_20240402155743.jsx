...
const ProductDetailTop = ({idProduct, loading}) => {
  ...
  const handleClickZoomIn = () => {
    const sliderItemActive = sliderItemRef.current.querySelector(
      ".slider-item-wrapper.swiper-slide-active"
    );
    sliderItemActive.style.transform = `scale(1.${zoom > 8 ? 8 : zoom})`;
    setZoom(zoom + 4);
  }
  const handleClickZoomOut = () => {
    ...
    sliderItemActive.style.transform = `scale(1.${zoom < 0 ? 0 : zoom})`;
    setZoom(zoom < 0 ? zoom : zoom - 4);
  }
    return (
      <>
        {loading ? (
                    ...
                    idProduct.map((item, index) => (
                      <div key={index}>
                        {item.image.map((src, index) => (
                          ...
                          >
                            <span className={"sliderItemPopup"}>
                              <img src={src} alt="" />
                            </span>
                          </SwiperSlide>
                        ))}
