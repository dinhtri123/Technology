...
setOpenPopupSearch(true);
  ...
  const handleShowSubMenu = (e) => {
    let getDataSubIcon = e.currentTarget.parentNode.querySelector("[data-sub-icon]");
    if (getDataSubIcon.classList.contains(styles.activeSubIcon)) {
      getDataSubIcon.classList.remove(styles.activeSubIcon);
    } else {
      getDataSubIcon.classList.add(styles.activeSubIcon);
    }
    if (subMenuRef.current.style.maxHeight) {
      ...
      menuMobileRef.current.style.maxHeight = `${subMenuRef.current.scrollHeight + menuMobileRef.current.scrollHeight}px`;
    }
  };
  return (
    <>
